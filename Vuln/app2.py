...
# ---------------- SQL INJECTION MONITORING (NON-BLOCKING) ----------------
...
def check_sql_injection():
    ...
    for key, value in request.args.items():
        if detect_sql_injection(str(value)):
            detected = True
            malicious_inputs.append(f"Query Param '{key}': {value}")
    ...
    if request.form:
        for key, value in request.form.items():
            if detect_sql_injection(str(value)):
                detected = True
                malicious_inputs.append(f"Form Field '{key}': {value}")
        ...
        try:
            ...
            for key, value in data.items():
                if isinstance(value, str) and detect_sql_injection(value):
                    detected = True
                    malicious_inputs.append(f"JSON Field '{key}': {value}")
        except:
            pass
    ...
    if detected:
        log_entry = {
            ...
            'blocked': False,  # Changed to False - we're not blocking
    ...
    # Return None to allow the request to continue normally
...
# ---------------- SQL INJECTION LOG ENDPOINTS ----------------
@app.route('/api/sql-logs')
def get_sql_logs():
    """Get SQL injection attack logs"""
    if 'user' not in session:
        return jsonify({"error": "Unauthorized"}), 403
    return jsonify(list(sql_attack_logs))
...
def get_sql_stats():
    ...
    return jsonify({
        ...
        'attacking_ips': ips,
        'recent_logs': list(sql_attack_logs)[-10:]
    })
...
if __name__ == "__main__":
    ...
    print("   • View attack logs at /api/sql-logs and /api/sql-stats")
